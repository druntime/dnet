...
use dnet::{codecs::BincodeCodec, udp::UdpTransport, utils::number};
...
pub async fn run(address: &str) -> Result<()> {
    ...
    let mut transport =
        UdpTransport::<_, _, number::Wrapper<u128, ()>, number::Wrapper<u128, Message>>::new(
            udp_socket,
